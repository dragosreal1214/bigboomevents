...
<body style="margin:0;background:#FBF6F4;font-family:Arial,Helvetica,sans-serif;color:#3B2E3A">
  <div style="max-width:560px;margin:0 auto;padding:24px">
    ...
    <p style="text-align:center;color:#6F6270;font-size:12px;margin-top:20px">
      BigBoomEvents · <a href="${config.publicUrl}" style="color:#6F6270">bigboomevents.ro</a>
    </p>
  </div>
</body></html>`;
...
const itemsTable = (items) => `
  <table style="width:100%;border-collapse:collapse;margin:8px 0 16px">
    ${items
      .map(
        (it) => `<tr>
          <td style="padding:8px 0;border-bottom:1px solid #ECE1E5">${it.product_name} × ${it.quantity}</td>
...
export function orderConfirmationToCustomer(order, items) {
  ...
  const inner = `
    <p>Salut, ${order.customer_name}! Îți mulțumim pentru comandă. 🎈</p>
    ...
    <p>Livrare la: ${order.ship_address}, ${order.ship_city}, ${order.ship_county}</p>
    ${order.delivery_date ? `<p>Livrare dorită: <strong>${fmtDate(order.delivery_date)}${order.delivery_slot ? ` · ${order.delivery_slot}` : ''}</strong></p>` : ''}
    ${order.gift_message ? `<p>Mesaj felicitare: „${order.gift_message}"</p>` : ''}
    <p style="color:#6F6270;font-size:13px">Te contactăm în curând pentru confirmarea detaliilor.</p>`;
  return {
    subject: `Comanda ${order.order_number} · BigBoomEvents`,
...
export function orderNotificationToAdmin(order, items) {
  const inner = `
    ...
    <p>Adresă: ${order.ship_address}, ${order.ship_city}, ${order.ship_county} ${order.ship_postcode || ''}</p>
    ${order.delivery_date ? `<p><strong>Livrare dorită: ${fmtDate(order.delivery_date)}${order.delivery_slot ? ` · ${order.delivery_slot}` : ''}</strong></p>` : ''}
    ${order.gift_message ? `<p>📝 Felicitare: „${order.gift_message}"</p>` : ''}
    ${order.notes ? `<p>Mențiuni: ${order.notes}</p>` : ''}`;
  return {
...
export function leadConfirmationToCustomer(lead) {
  const inner = `
    ...
    ${lead.event_type ? `<p>Tip eveniment: <strong>${lead.event_type}</strong></p>` : ''}
    <p style="color:#6F6270;font-size:13px">Dacă e ceva urgent, ne poți suna direct.</p>`;
  return {
    subject: 'Am primit cererea ta · BigBoomEvents',
...
export function leadNotificationToAdmin(lead) {
  const inner = `
    ...
    <p>${lead.name} · ${lead.email} ${lead.phone ? '· ' + lead.phone : ''}</p>
    ${lead.event_date ? `<p>Data: ${lead.event_date}</p>` : ''}
    ${lead.event_type ? `<p>Tip: ${lead.event_type}</p>` : ''}
    ${lead.message ? `<p>Mesaj: ${lead.message}</p>` : ''}`;
  return {
    subject: `✨ Lead nou — ${lead.name}`,
    html: shell('Lead nou', inner),
    text: `Lead nou de la ${lead.name} (${lead.email}).`,
  };
}
